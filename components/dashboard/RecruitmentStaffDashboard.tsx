...
export default function RecruitmentStaffDashboard({
    ...
    const [formData, setFormData] = useState<{ departmentName: string; officeName: string }>();
    ...
    function handleFormSubmit(event: React.FormEvent<HTMLFormElement>) {
        ...
        const representativeFormData = {
            departmentName: formData.get("select_department_representative") as string,
            ...
            <div className="w-full overflow-x-auto py-16">
                {Array.isArray(users) &&
                    users.map(({ id, name, firstName, lastName, email, role }) => (
                        ...
                        >
                            <DIV title="Name" name={name} />
                            <DIV title="First Name" name={firstName} />
                            <DIV title="Last Name" name={lastName} />
                            <DIV title="Email" name={email} />
                            <div className="flex items-center gap-10">
                                <DIV title="Role" name={role} />
                                <div className="flex flex-col flex-1 p-3 bg-white rounded-lg shadow-xl">
                                    <label className="font-bold">Select representative</label>
...
function DIV({ title, name }: { title: string; name: string }) {
    return (
