...
export default async function SidebarApplicant({
  department,
}: {
  department: string;
}) {
  const { departmentApplicants, officeApplicants } =
    await getAllApplicantsLists(department);
...
function DisplayApplicant({
  ...
  return (
    <div className="w-full p-3.5 bg-amber-500 hover:text-white hover:bg-red-900 rounded-lg cursor-pointer hover:scale-95 transition duration-200">
      <Link
        href={`/${department || officeName}/applicants/${id}`}
        scroll={false}
      >
        <div>
